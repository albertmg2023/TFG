...
def launch_setup(context: LaunchContext, *args, **kwargs):
  ...
  icp_odometry_parameters = {
    ...
    'deskewing': False, # If fixed_frame_id is set, we do deskewing externally below
    'odom_frame_id': 'watcher_1/odom',
  ...
  if localization:
    ...
    rtabmap_parameters['Mem/InitWMWithAllNodes'] = 'False'
  else:
    arguments.append('-d') # This will delete the previous database (~/.ros/rtabmap.db)
  
  remappings = [('odom', 'watcher_1/rb_watcher_odom')]
  if imu_used:
  ...
  nodes = [
    ...
    Node(
      ...
      parameters=[shared_parameters, rtabmap_parameters, {'subscribe_rgbd': rgbd_image_used,'topic_queue_size': 10,
                   'sync_queue_size': 10}],
      remappings=remappings + [('scan_cloud', lidar_topic_deskewed)],
      arguments=arguments), 
    ...
    Node(
      ...
      remappings=remappings + [('scan_cloud', 'watcher_1/rb_watcher_LIDAR_3D')])
  ]
...
def generate_launch_description():
  return LaunchDescription([
    ...
    DeclareLaunchArgument(
      'frame_id', default_value='watcher_1/robot_base_link',
    ...
    DeclareLaunchArgument(
      'lidar_topic', default_value='/watcher_1/rb_watcher_LIDAR_3D',
    ...
    DeclareLaunchArgument(
      'rgbd_image_topic', default_value='/rgbd_image',
